...
class TKeyValueCache {
    ...
    private static final String TAG = "TKeyValueCache";
    private static final String DIR_NAME = "t_key_value_cache";
    /**
    ...
    static void initialize(File path) {
        if (!path.isDirectory() && !path.mkdir()) {
            throw new RuntimeException("Could not create TKeyValueCache directory");
        }
        directory = path;
    }
    ...
    static void saveToKeyValueCache(String key, String value) {
        synchronized (MUTEX_IO) {
            ...
            try {
                TFileUtils.writeByteArrayToFile(f, value.getBytes("UTF-8"));
