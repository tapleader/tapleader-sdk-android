...
/**
 * Created by mehdi akbarian on 2017-03-04.
 * profile: http://ir.linkedin.com/in/mehdiakbarian
 */
class WKeyValueCache {

  private static final String TAG = "WKeyValueCache";
  private static final String DIR_NAME = "WeclickKeyValueCache";

  // We limit the cache to 2MB because that's about what the default browser
  // uses.
  static final int DEFAULT_MAX_KEY_VALUE_CACHE_BYTES = 2 * 1024 * 1024;
  ...
  static int maxKeyValueCacheBytes = DEFAULT_MAX_KEY_VALUE_CACHE_BYTES;
  ...
  /* package for tests */ static void initialize(File path) {
    if (!path.isDirectory() && !path.mkdir()) {
      throw new RuntimeException("Could not create WKeyValueCache directory");
    }
    directory = path;
  }
  ...
  static void saveToKeyValueCache(String key, String value) {
    synchronized (MUTEX_IO) {
      ...
      try {
        WFileUtils.writeByteArrayToFile(f, value.getBytes("UTF-8"));
  ...
  static String loadFromKeyValueCache(final String key, final long maxAgeMilliseconds) {
    synchronized (MUTEX_IO) {
      ...
      try {
        ...
        WLog.e(TAG, "error reading from cache", e);
        return null;
      }
    }
  }
  ...
  static JSONObject jsonFromKeyValueCache(String key, long maxAgeMilliseconds) {
    ...
    try {
      ...
      WLog.e(TAG, "corrupted cache for " + key, e);
